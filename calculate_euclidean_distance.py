...
class EuclideanDistance:
    def __init__(self, _from_vector, _to_vector):
        self.from_vector = _from_vector.split('-')[0]
        self.to_vector_label = _to_vector.split('-')[-1] if len(_to_vector.split('-')) > 1 else "no-label"
        self.to_vector = _to_vector.split('-')[0]
        self.from_vector_list = [int(_vector_coordinate_value) for _vector_coordinate_value in self.from_vector.split(",")]
        self.to_vector_list = [int(_vector_coordinate_value) for _vector_coordinate_value in self.to_vector.split(",")]

    def execute(self):
        data = [[self.from_vector, self.to_vector, self.to_vector_label, self.compute_euclidean_distance()]]
        ed_df = pd.DataFrame(data, columns=['FromVector', 'ToVector', 'Label', 'EuclideanDistance'])
